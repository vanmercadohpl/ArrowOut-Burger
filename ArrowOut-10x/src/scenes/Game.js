...
export class Game extends Phaser.Scene {
  ...
  createArrowGame() {
    ...
    this.arrowGame = new ArrowBoxGame(
      ...
      {
        onCollision: () => {
          this.handleArrowActionResolved();
          if (!this.endSceneShown) {
            this.playLastHeartCollisionBlink();
          }
        },
        onCompleteArrow: () => this.handleArrowActionResolved(),
  ...
  handleArrowActionResolved() {
    if (
      this.endSceneShown ||
      this.failOverlayShown ||
      this.winOverlayShown ||
      this.arrowActionCount >= MAX_ARROW_ACTIONS
    ) {
      return;
    }

    this.arrowActionCount += 1;

    if (this.arrowActionCount >= MAX_ARROW_ACTIONS) {
